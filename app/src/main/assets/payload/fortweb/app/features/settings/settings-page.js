...
export function renderSettingsPage({ vault, settings, onDeleteVault }) {
    return {
        ...
        render(container) {
            ...
            defaultsGrid.append(detailItem("Vault", vault.alias), detailItem("Temporary Datastore", enabledLabel(settings.tempDatastore)), detailItem("Key Algorithm", settings.keyAlgorithm), detailItem("Key Tier", settings.keyTier), detailItem("Witness Profile", settings.witnessProfile));
            ...
            runtimeGrid.append(detailItem("Storage Backend", settings.storageBackend), detailItem("Runtime Status", settings.runtimeStatus));
