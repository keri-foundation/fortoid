...
const RUNTIME_ORIGIN_VERSION = 1;
...
export class RuntimeOriginContractError extends Error {
    code = "INVALID_RUNTIME_ORIGIN_CONTRACT";
    constructor(message) {
        super(message);
...
function requireBooleanOrUnknown(value, field) {
    assertRuntimeContract(typeof value === "boolean" || value === "unknown", `${field} must be a boolean or "unknown".`);
    return value;
}
function requireEnum(value, allowed, field) {
    assertRuntimeContract(typeof value === "string" && allowed.includes(value), `${field} was invalid.`);
    return value;
}
...
function parseContractUrl(value, field) {
    try {
...
function validateIosCommonCapabilities(contract) {
    assertRuntimeContract(contract.capabilities.httpsLikeAssetOrigin === false, "Bundled iOS runtime origin contract must set httpsLikeAssetOrigin=false.");
    assertRuntimeContract(contract.capabilities.implicitBlobOriginSafe === false, "Bundled iOS runtime origin contract must set implicitBlobOriginSafe=false.");
    assertRuntimeContract(contract.capabilities.networkAllowed === false, "Bundled iOS runtime origin contract must set networkAllowed=false.");
    assertRuntimeContract(contract.capabilities.bundledAssetsOnly === true, "Bundled iOS runtime origin contract must set bundledAssetsOnly=true.");
}
function validateIosAppLocalContract(contract) {
    assertRuntimeContract(normalizeBaseUrl(contract.documentOrigin) === IOS_APP_LOCAL_ORIGIN, "Bundled iOS app-local runtime origin contract must use document origin app://local.");
    assertRuntimeContract(normalizeBaseUrl(contract.appBaseUrl) === IOS_APP_LOCAL_ORIGIN, "Bundled iOS app-local runtime origin contract must use app base URL app://local.");
    assertRuntimeContract(normalizeBaseUrl(contract.storage.originPartition) === IOS_APP_LOCAL_ORIGIN, "Bundled iOS app-local runtime origin contract must use origin partition app://local.");
    for (const [field, value] of [
        ["entryUrl", contract.entryUrl],
        ["workerUrl", contract.workerUrl],
        ["configUrl", contract.configUrl],
    ]) {
        assertRuntimeContract(value.startsWith(`${IOS_APP_LOCAL_ORIGIN}/`), `Bundled iOS app-local runtime origin contract must use app://local URLs for ${field}.`);
        assertRuntimeContract(!value.startsWith("http://") && !value.startsWith("https://"), `Bundled iOS app-local runtime origin contract must not use network URLs for ${field}.`);
        assertRuntimeContract(!value.includes("localhost") && !value.includes(IOS_LOOPBACK_HOST), `Bundled iOS app-local runtime origin contract must not use loopback URLs for ${field}.`);
    }
...
function validateIosLoopbackContract(contract) {
    const documentOrigin = parseContractUrl(contract.documentOrigin, "Runtime origin contract.documentOrigin");
    assertRuntimeContract(contract.documentOrigin.startsWith(`http://${IOS_LOOPBACK_HOST}:`), "Bundled iOS loopback runtime origin contract must use canonical 127.0.0.1 HTTP origin.");
    assertRuntimeContract(documentOrigin.protocol === IOS_LOOPBACK_PROTOCOL && documentOrigin.hostname === IOS_LOOPBACK_HOST, "Bundled iOS loopback runtime origin contract must use 127.0.0.1 over HTTP.");
    assertRuntimeContract(documentOrigin.port.length > 0 && documentOrigin.port !== "0", "Bundled iOS loopback runtime origin contract must use an explicit non-zero port.");
    assertRuntimeContract(documentOrigin.pathname === "/", "Bundled iOS loopback runtime origin contract must not include a document path.");
    ...
    assertRuntimeContract(appBaseSegments.length === 2 && appBaseSegments[0] === IOS_LOOPBACK_PATH_PREFIX, "Bundled iOS loopback runtime origin contract must use the hardened path prefix.");
    const nonce = appBaseSegments[1];
    assertRuntimeContract(IOS_LOOPBACK_NONCE_PATTERN.test(nonce), "Bundled iOS loopback runtime origin contract must include a non-trivial nonce path segment.");
    ...
    for (const [field, value] of [
        ...
        assertRuntimeContract(url.pathname.startsWith(noncePathPrefix), `Bundled iOS loopback runtime origin contract must keep ${field} under the nonce path prefix.`);
    }
...
function rejectSecretBearingFields(value, path = "contract") {
...
function validateOptionalUrlObject(value, field) {
...
function validateIosBundledOfflineContract(contract) {
    assertRuntimeContract(contract.platform === "ios-wkwebview", "Bundled iOS runtime origin contract must declare platform ios-wkwebview.");
    assertRuntimeContract(contract.mode === "bundled-offline", "Bundled iOS runtime origin contract must declare mode bundled-offline.");
...
export function validateRuntimeOriginContract(rawValue) {
    const root = requireObject(rawValue, "Runtime origin contract");
    rejectSecretBearingFields(root);
    const platform = requireEnum(root.platform, ["ios-wkwebview", "android-webview", "browser-dev"], "Runtime origin contract.platform");
    const mode = requireEnum(root.mode, ["bundled-offline", "browser-dev"], "Runtime origin contract.mode");
    ...
    const contract = {
        schema: requireEnum(root.schema, [RUNTIME_ORIGIN_SCHEMA], "Runtime origin contract.schema"),
        version: 1,
        platform,
        mode,
        ...
        storage: {
            ...
            indexedDbRequired: requireBooleanOrUnknown(storage.indexedDbRequired, "Runtime origin contract.storage.indexedDbRequired"),
            originPartition: normalizeBaseUrl(requireAbsoluteUrl(storage.originPartition, "Runtime origin contract.storage.originPartition")),
        },
        capabilities: {
            customScheme: requireBoolean(capabilities.customScheme, "Runtime origin contract.capabilities.customScheme"),
            httpsLikeAssetOrigin: requireBoolean(capabilities.httpsLikeAssetOrigin, "Runtime origin contract.capabilities.httpsLikeAssetOrigin"),
            implicitBlobOriginSafe: requireBooleanOrUnknown(capabilities.implicitBlobOriginSafe, "Runtime origin contract.capabilities.implicitBlobOriginSafe"),
            networkAllowed: false,
            bundledAssetsOnly: true,
        },
        pyodide: validateOptionalUrlObject(root.pyodide, "Runtime origin contract.pyodide"),
        wheels: validateOptionalUrlObject(root.wheels, "Runtime origin contract.wheels"),
    };
...
export function readWindowRuntimeOriginContract(targetWindow = window) {
...
export function describeRuntimeOriginContract(contract) {
    return {
...
export function isPlainLocalBrowserDevLocation(location) {
    const protocol = location.protocol;
    const hostname = location.hostname;
    return ((protocol === "http:" || protocol === "https:") &&
        (hostname === "localhost" || hostname === "127.0.0.1" || hostname === "::1" || hostname === "[::1]"));
}
export function isRuntimeOriginContractRequired(location) {
