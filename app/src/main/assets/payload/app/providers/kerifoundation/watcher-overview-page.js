...
function renderWatcherTable(watchers) {
    ...
    const table = renderPaginatedTable({
        ...
        emptyText: "This KF account is onboarded locally, but the boot service did not return any watcher rows yet.",
    });
    return {
        render(container) {
...
export function renderWatcherOverviewPage({ vault, bootstrapState, watchers, watcherError, onRefreshStatuses, }) {
    ...
    return {
        ...
        render(container) {
            ...
            summary.append(detailItem("Account Alias", account?.accountAlias || "\u2014"), detailItem("Account AID", account?.accountAid || "\u2014"), detailItem("Region", account?.regionName || account?.regionId || "\u2014"), detailItem("Watcher Policy", account?.watcherRequired ? "Required" : "Optional"), detailItem("Boot Service", bootstrapState.bootUrl || account?.bootUrl || "\u2014"), detailItem("Boot Server AID", account?.bootServerAid || "Pending verification"));
